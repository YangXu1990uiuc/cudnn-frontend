...
"""sm100 CTA_1 GEMM kernel with mainloop fusion: persistent + double-TMEM + CLC dynamic scheduler.

Single-CTA MMA plus 4 dedicated mainloop warps that apply a unary / scalar op
chain to the A and/or B operand tile in SMEM (in place, swizzle-preserved)
before the MMA reads it. The compiler picks this template when the graph has
mainloop fusion and ``TileConfig.cta_group == 1``.
...
Warp layout (12 warps × 32 = 384 threads/CTA — 8 + 4 mainloop):
  ...
  warps 8–11 : mainloop fusion (transform A/B operand tile in SMEM before the MMA)  — setmaxnreg.inc 216
"""
...
def _kernel(
    ...
    if warp_idx == 0:
        for i in range(ab_stages):
            ...
            if elect_one:
                nvvm.mbarrier_init(mainloop_full_mbar_ptr.subview(i), num_mainloop_warps)
            if elect_one:
                            ...
                            for m_group in cutlass.range_constexpr(cta_tile_mnk[0] // a_tma_group_elems):
                                if elect_one:
                                    nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                        sA_tma_dst.subview(m_group * a_tma_group_elems * cta_tile_mnk[2]),
                                        tma_a_desc.get_ptr(),
                                        (
                    ...
                    if m_rank == 0:
                        if cutlass.const_expr(b_is_n_major):
                            for n_group in cutlass.range_constexpr(cta_tile_mnk[1] // b_tma_group_elems):
                                if elect_one:
                                    nvvm.cp_async_bulk_tensor_shared_cluster_global(
                                        sB_tma_dst.subview(n_group * b_tma_group_elems * cta_tile_mnk[2]),
                                        tma_b_desc.get_ptr(),
                                        (
    ...
    if warp_idx == mma_warp_id:
        ...
        while is_valid != 0:
            ...
            tmem_addr_int = (base_row_id << 16) | acc_col_id
            tmem_addr_for_stage = cutlass.inttoptr(tmem_addr_int, 6, cutlass.Int32)
            ...
            for k_tile_idx in range(num_k_tiles):
                ...
                if cutlass.const_expr(not mainloop_fuse_a):
                    while not nvvm.mbarrier_try_wait_parity(a_full_mbar_ptr.subview(stage), a_full_phase_bit, time_limit=10_000_000):
                        pass
                if cutlass.const_expr(not mainloop_fuse_b):
                    while not nvvm.mbarrier_try_wait_parity(b_full_mbar_ptr.subview(stage), b_full_phase_bit, time_limit=10_000_000):
                        pass

                sA_stage = smem_a.subview(sA_elems * stage)
                sB_stage = smem_b.subview(sB_elems * stage)

                desc_a_base = cutlass.experimental.primitives.Tcgen05SmemDesc.build(
                ...
                desc_b_base = cutlass.experimental.primitives.Tcgen05SmemDesc.build(
    ...
    if warp_idx >= mainloop_warp_id_start:
        ...
        # Full per-thread rounds + the remainder vectors (threads < ml_tail_x
        # each take one extra). A floor alone would leave the tile's tail
        # untransformed whenever sX_elems isn't a multiple of 128 vectors —
        # e.g. any cta_tile_n with N*K_BYTES % 2048 != 0 on the B side.
                ...
                if cutlass.const_expr(mainloop_fuse_a):
                    ...
                    if cutlass.const_expr(mainloop_a_cast):
                        ...
                        for chunk in cutlass.range_constexpr(ml_chunks_a):
                            ...
                            if cutlass.const_expr(a_is_m_major):
                                ml_k_of_L = (ml_L % (a_tma_group_elems * cta_tile_mnk[2])) // a_tma_group_elems
                            else:
                                ml_k_of_L = ml_L % cta_tile_mnk[2]
...
def _host(
    ...
    if cutlass.const_expr(a_is_m_major):
        ...
        a_box_dims = (a_tma_group_elems, cta_tile_mnk[2], 1)
        ...
        a_box_dims = (cta_tile_mnk[0], cta_tile_mnk[2], 1)
    ...
    if cutlass.const_expr(b_is_n_major):
        ...
        b_box_dims = (b_tma_group_elems, cta_tile_mnk[2], 1)
        ...
        b_box_dims = (cta_tile_mnk[1], cta_tile_mnk[2], 1)
