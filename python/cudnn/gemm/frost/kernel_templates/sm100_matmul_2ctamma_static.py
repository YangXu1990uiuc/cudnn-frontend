...
"""sm100 cta_group=2 GEMM kernel: cluster MMA + STATIC scheduler (no CLC) + double-TMEM.

2-CTA MMA pair (leader issues the MMA, follower consumes); the grid launches one
cluster per cgrp_tile, so each pair processes exactly the one cgrp_tile its blockIdx
maps to and exits — no CLC ring, best for small ≤1-wave shapes. Compiler picks this
when TileConfig.cta_group == 2 (static).
