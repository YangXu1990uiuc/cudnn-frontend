...
"""sm100 cta_group=2 GEMM kernel: cluster MMA + CLC dynamic scheduler + double-TMEM.

2-CTA MMA cluster pair: the leader CTA issues the MMA while the follower
CLC-consumes only; both CTAs load their operand slice. Persistent, with a
dedicated scheduler warp pumping a 2-slot CLC ring for cross-tile pipelining.
The compiler picks this template when ``TileConfig.cta_group == 2``.
...
def _kernel(
    ...
    # TMEM: per-pair (CTA_2 group), double-buffered along N.
        ...
        if cutlass.const_expr(a_is_m_major):
            tma_a_desc_list.append(
                _tma.create_tensor_map_tiled(
                    ...
                    box_dims=[a_tma_group_elems, cgrp_tile_mnk[2], 1],
