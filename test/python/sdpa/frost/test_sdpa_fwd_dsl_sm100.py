...
def test_dsl_sm100_thd_padded_stats_in_a_non_self_inverse_layout():
    """A padded Stats whose (b, h, s_max) storage order is (h, b, s_max): the
    order and its inverse differ, so a kernel compiled with the wrong one
    writes through other strides. Rows must equal the contiguous layout's."""
    _require_dsl()
    from cudnn.sdpa.fwd.api_dsl import SdpaFwdDslSm100
    ...
    run(ref)
    hbs = torch.full((h, b, s), float("nan"), dtype=torch.float32, device="cuda").permute(1, 0, 2)  # (b, h, s) view, strides (s, b*s, 1)
    api = run(hbs)
    assert tuple(hbs.stride()) == (s, b * s, 1)
    assert SdpaFwdDslSm100._thd_padded_lse_order(api) == (2, 3, 1, 0)
    for i, n in enumerate(lens.tolist()):
        torch.testing.assert_close(hbs[i, :, :n], ref[i, :, :n], atol=0, rtol=0)
        assert torch.isneginf(hbs[i, :, n:]).all(), f"batch {i}: rows past the length are not -inf"
