...
def test_discrete_grouped_bf16_layout_and_changed_weight_graph(major, dynamic):
    ...
    plan.compile()
    workspace = torch.empty(plan.scratch_workspace_bytes(), device=a.device, dtype=torch.uint8)

    def run():
        plan.execute(as_mkl(a), c, d, offsets, alpha, b_ptrs=ptrs, prob_tensor=prob, workspace=workspace)
